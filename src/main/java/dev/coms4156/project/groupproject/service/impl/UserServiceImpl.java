...
import dev.coms4156.project.groupproject.utils.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public void register(RegisterRequest req) {
        ...
        u.setName(StringUtils.hasText(req.getName()) ? req.getName() : SystemConstants.DEFAULT_NICK_PREFIX + TokenUtil.shortUuid());
    ...
    public void updateMe(UpdateProfileRequest req) {
        ...
        updateById(user);
        // also refresh session snapshot
        UserView newView = new UserView(user.getId(), user.getName());
        SessionUtil.refreshCurrentUserSnapshot(redis, newView);
    }
    ...
    private TokenPair issueTokens(User user) {
        ...
        redis.opsForValue().set(RedisKeys.accessTokenKey(access), Jsons.toJson(uv), Duration.ofHours(RedisKeys.ACCESS_TOKEN_TTL_HOURS));
        redis.opsForValue().set(RedisKeys.refreshTokenKey(refresh), Jsons.toJson(uv), Duration.ofDays(RedisKeys.REFRESH_TOKEN_TTL_DAYS));
